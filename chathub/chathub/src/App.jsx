...
import { getResponse, API_CALLERS } from "./api";
...
function AuthScreen({ onLogin }) {
  ...
  const submit = async () => {
    ...
    if (isSignup) {
      ...
      const ud = { username: username.trim().toLowerCase(), passwordHash: hashed, createdAt: new Date().toISOString() };
      storageSet(ukey, ud);
  ...
  const inp = { width: "100%", padding: "11px 14px", borderRadius: 10, border: "1px solid #2A2A30", background: "#131316", color: "#E8E6E3", fontSize: 14, fontFamily: "inherit", outline: "none", transition: "border-color 0.2s" };
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "radial-gradient(ellipse at 30% 20%, #1A1915 0%, #131316 60%)" }}>
      <div style={{ width: 400, animation: "fadeUp 0.5s ease-out" }}>
        ...
        <div style={{ background: "#1A1A1F", border: "1px solid #2A2A30", borderRadius: 16, padding: 32 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            <div><label style={{ fontSize: 11, fontWeight: 600, color: "#888", textTransform: "uppercase", letterSpacing: "0.08em", display: "block", marginBottom: 6 }}>Usuário</label><input value={username} onChange={e => setUsername(e.target.value)} placeholder="seu_usuario" onKeyDown={e => e.key==="Enter"&&submit()} style={inp} /></div>
            <div><label style={{ fontSize: 11, fontWeight: 600, color: "#888", textTransform: "uppercase", letterSpacing: "0.08em", display: "block", marginBottom: 6 }}>Senha</label><input value={password} onChange={e => setPassword(e.target.value)} type="password" placeholder="••••••" onKeyDown={e => e.key==="Enter"&&submit()} style={inp} /></div>
            {isSignup && <div><label style={{ fontSize: 11, fontWeight: 600, color: "#888", textTransform: "uppercase", letterSpacing: "0.08em", display: "block", marginBottom: 6 }}>Confirmar Senha</label><input value={confirmPw} onChange={e => setConfirmPw(e.target.value)} type="password" placeholder="••••••" onKeyDown={e => e.key==="Enter"&&submit()} style={inp} /></div>}
...
function ApiSettingsModal({ apiKeys, onSave, onClose }) {
  ...
  const updateKey = (pid, val) => setKeys(prev => ({ ...prev, [pid]: val }));
  ...
  const testKey = async (pid) => {
    ...
    try {
      const testMsgs = [{ role: "user", content: "Diga apenas: OK" }];
      await API_CALLERS[pid](testMsgs, key);
      setTestResults(prev => ({ ...prev, [pid]: "success" }));
    } catch (err) {
      setTestResults(prev => ({ ...prev, [pid]: err.message || "Erro" }));
    }
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", backdropFilter: "blur(6px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 999, animation: "fadeUp 0.25s ease-out" }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ width: 540, maxHeight: "85vh", background: "#1A1A1F", border: "1px solid #2A2A30", borderRadius: 18, overflow: "hidden", display: "flex", flexDirection: "column" }}>
        <div style={{ padding: "20px 24px 16px", borderBottom: "1px solid #2A2A30", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div>
            <h2 style={{ fontSize: 18, fontWeight: 700, color: "#F0EDE8", margin: 0 }}>⚙️ Configurações de API</h2>
            <p style={{ fontSize: 12, color: "#777", marginTop: 4 }}>Conecte seus providers para respostas reais</p>
          </div>
          <button onClick={onClose} style={{ background: "none", border: "1px solid #2A2A30", color: "#888", cursor: "pointer", width: 32, height: 32, borderRadius: 8, fontSize: 16, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "inherit" }}>×</button>
        </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "16px 24px 24px" }}>
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            {PROVIDERS.map(p => {
              ...
              return (
                <div key={p.id} style={{ background: "#131316", border: `1px solid ${hasKey ? p.color+"30" : "#2A2A30"}`, borderRadius: 14, padding: "16px 18px", transition: "all 0.2s" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 14, fontWeight: 600, color: "#F0EDE8", display: "flex", alignItems: "center", gap: 8 }}>
                        {p.name}
                        {hasKey && <span style={{ fontSize: 9, padding: "2px 8px", borderRadius: 10, background: "#10A37F20", color: "#1ED99C", fontWeight: 600 }}>CONECTADO</span>}
                      </div>
                  ...
                  <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                    <div style={{ flex: 1, position: "relative" }}>
                      <input value={keys[p.id] || ""} onChange={e => updateKey(p.id, e.target.value)}
                        type={visible ? "text" : "password"} placeholder={p.placeholder}
                        style={{ width: "100%", padding: "9px 36px 9px 12px", borderRadius: 8, border: `1px solid ${isSuccess?"#10A37F40":isError?"#FF4D4F40":"#2A2A30"}`, background: "#1A1A1F", color: "#E8E6E3", fontSize: 12, fontFamily: "'JetBrains Mono', monospace", outline: "none" }} />
                      <button onClick={() => setShowKey(prev => ({ ...prev, [p.id]: !prev[p.id] }))}
                        style={{ position: "absolute", right: 8, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", color: "#666", cursor: "pointer", fontSize: 13, padding: "2px" }}>
                        {visible ? "🙈" : "👁"}
                      </button>
                    </div>
                    <button onClick={() => testKey(p.id)} disabled={isTesting || !keys[p.id]}
                      style={{ padding: "9px 14px", borderRadius: 8, border: "none", background: isTesting?"#333":p.color+"25", color: isTesting?"#666":p.accent, cursor: isTesting||!keys[p.id]?"not-allowed":"pointer", fontSize: 11, fontWeight: 600, fontFamily: "inherit", whiteSpace: "nowrap" }}>
                      {isTesting ? "..." : "Testar"}
                    </button>
                    {keys[p.id] && (
                      <button onClick={() => { updateKey(p.id, ""); setTestResults(prev => ({ ...prev, [p.id]: null })); }}
                        style={{ padding: "9px 10px", borderRadius: 8, border: "1px solid #FF4D4F30", background: "transparent", color: "#FF6B6B", cursor: "pointer", fontSize: 11, fontFamily: "inherit" }}>✕</button>
                    )}
                  </div>
                  {result && <div style={{ marginTop: 8, padding: "6px 10px", borderRadius: 6, fontSize: 11, background: isSuccess?"#10A37F12":"#FF4D4F12", color: isSuccess?"#1ED99C":"#FF6B6B", border: `1px solid ${isSuccess?"#10A37F25":"#FF4D4F25"}` }}>{isSuccess ? "✓ Conexão bem-sucedida!" : `✗ ${result}`}</div>}
                </div>
              );
            })}
          </div>
          <div style={{ marginTop: 20, padding: "14px 16px", borderRadius: 10, background: "#C9A96E08", border: "1px solid #C9A96E20" }}>
            ...
            <div style={{ fontSize: 11, color: "#888", lineHeight: 1.6 }}>Suas API keys são salvas localmente no navegador e nunca são enviadas a terceiros. As chamadas passam pelo proxy do servidor para evitar problemas de CORS.</div>
          </div>
        </div>
...
function ChatListItem({ chat, prov, isAct, onClick, onDelete, onRename }) {
  ...
  return (
    <div onClick={onClick} style={{ display: "flex", alignItems: "center", gap: 8, padding: "9px 8px", borderRadius: 10, background: isAct?prov.color+"12":"transparent", cursor: "pointer", border: isAct?`1px solid ${prov.color}25`:"1px solid transparent", transition: "all 0.15s" }}>
      ...
      <button onClick={e => { e.stopPropagation(); onDelete(); }} style={{ background: "none", border: "none", color: "#444", cursor: "pointer", fontSize: 13, padding: "2px 4px", borderRadius: 4, opacity: 0, transition: "opacity 0.2s" }}
...
function ChatApp({ username, onLogout }) {
  ...
  const saveChatList = useCallback((list) => {
    clearTimeout(saveTimer.current);
    saveTimer.current = setTimeout(() => storageSet(chatsKey, list), 300);
  }, [chatsKey]);
  ...
  const sendMessage = async () => {
    ...
    const autoTitle = isFirst ? text.slice(0,50) + (text.length > 50 ? "..." : "") : null;
    ...
    <div style={{ display: "flex", height: "100vh", width: "100%", background: "#131316", color: "#E8E6E3", overflow: "hidden" }}>
      ...
      <div style={{ width: 280, minWidth: 280, background: "#18181C", borderRight: "1px solid #2A2A30", display: "flex", flexDirection: "column" }}>
        <div style={{ padding: "14px 14px 12px", borderBottom: "1px solid #2A2A30" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            ...
            <div style={{ display: "flex", gap: 4 }}>
              <button onClick={() => setShowSettings(true)} title="APIs"
                style={{ background: "none", border: "1px solid #2A2A30", color: "#888", cursor: "pointer", padding: "4px 8px", borderRadius: 6, fontSize: 13, fontFamily: "inherit", position: "relative" }}>
                ⚙️{connectedCount > 0 && <span style={{ position: "absolute", top: -4, right: -4, width: 14, height: 14, borderRadius: "50%", background: "#10A37F", color: "#fff", fontSize: 8, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center" }}>{connectedCount}</span>}
              </button>
        ...
        <div style={{ padding: "10px 10px 2px" }}>
          <button onClick={() => createNewChat()} style={{ width: "100%", padding: "9px 12px", borderRadius: 10, border: "1px dashed #3A3A42", background: "transparent", color: "#C9A96E", cursor: "pointer", fontSize: 12, fontWeight: 600, fontFamily: "inherit", display: "flex", alignItems: "center", gap: 8 }}>
            <span style={{ fontSize: 17, lineHeight: 1 }}>+</span> Novo Chat
          </button>
        </div>
          ...
          {sidebarTab === "chats" ? (
            <>
              ...
              {filteredChats.length === 0 ? (
                <div style={{ textAlign: "center", padding: "36px 10px", color: "#555", fontSize: 12, lineHeight: 1.7 }}>{searchQuery ? "Nenhum chat encontrado." : <>Nenhum chat ainda.<br />Clique em <b style={{ color: "#C9A96E" }}>Novo Chat</b>.</>}</div>
              ) : (
                <div style={{ display: "flex", flexDirection: "column", gap: 2, marginTop: 4 }}>
                  {filteredChats.map(chat => {
                    const prov = PROVIDERS.find(p => p.id === chat.providerId) || PROVIDERS[0];
                    return <ChatListItem key={chat.id} chat={chat} prov={prov} isAct={activeChatId===chat.id}
                      onClick={() => { setActiveChatId(chat.id); setActiveProvider(chat.providerId||"claude"); setBroadcastMode(!!chat.broadcastMode); }}
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 4, marginTop: 4 }}>
              <button onClick={() => setBroadcastMode(!broadcastMode)} style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 10px", borderRadius: 10, border: broadcastMode?"1px solid #E8D5B730":"1px solid transparent", background: broadcastMode?"#E8D5B710":"transparent", color: broadcastMode?"#E8D5B7":"#888", cursor: "pointer", fontSize: 12, fontWeight: 600, fontFamily: "inherit", width: "100%", textAlign: "left" }}>
                <span style={{ fontSize: 15 }}>📡</span> Broadcast
                <span style={{ marginLeft: "auto", fontSize: 9, padding: "2px 7px", borderRadius: 10, background: broadcastMode?"#E8D5B730":"#2A2A30", color: broadcastMode?"#E8D5B7":"#666" }}>{broadcastMode?"ON":"OFF"}</span>
              </button>
              ...
              {PROVIDERS.map(pr => {
                const isA = !broadcastMode && activeProvider === pr.id;
                const conn = isConnected(pr.id);
                return (
                  <button key={pr.id} onClick={() => { if (!broadcastMode) setActiveProvider(pr.id); }}
                    style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 10px", borderRadius: 10, border: "none", background: isA?pr.color+"15":"transparent", cursor: broadcastMode?"default":"pointer", opacity: broadcastMode?0.4:1, fontFamily: "inherit", width: "100%", textAlign: "left" }}>
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 12, fontWeight: isA?600:500, color: isA?"#F0EDE8":"#999", display: "flex", alignItems: "center", gap: 6 }}>{pr.name}<span style={{ width: 5, height: 5, borderRadius: "50%", background: conn?"#1ED99C":"#555" }} /></div>
                      <div style={{ fontSize: 10, color: "#555" }}>{pr.tagline}{conn?"":" · sem key"}</div>
                    </div>
                  </button>
                );
              })}
              <button onClick={() => setShowSettings(true)} style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 10px", borderRadius: 10, border: "1px dashed #3A3A42", background: "transparent", color: "#888", cursor: "pointer", fontSize: 11, fontFamily: "inherit", width: "100%", textAlign: "left", marginTop: 4 }}>⚙️ Configurar API Keys</button>
            </div>
          )}
        </div>
        <div style={{ padding: "8px 12px", borderTop: "1px solid #2A2A30", fontSize: 10, color: "#444" }}>{connectedCount}/{PROVIDERS.length} providers conectados</div>
      </div>

      {/* MAIN */}
      <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
        <div style={{ padding: "11px 20px", borderBottom: "1px solid #2A2A30", display: "flex", alignItems: "center", gap: 12, background: "#18181C90", backdropFilter: "blur(12px)" }}>
          {broadcastMode ? (
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <div>
                <div style={{ fontSize: 14, fontWeight: 600, display: "flex", alignItems: "center", gap: 6 }}>{ap.name}<span style={{ width: 6, height: 6, borderRadius: "50%", background: isConnected(ap.id)?"#1ED99C":"#666" }} /></div>
                <div style={{ fontSize: 11, color: "#777" }}>{ap.tagline} · {ap.model}</div>
              </div>
            </div>
          )}
          <div style={{ marginLeft: "auto", display: "flex", gap: 6 }}>
            {!isConnected(activeProvider) && !broadcastMode && (
              <button onClick={() => setShowSettings(true)} style={{ padding: "5px 12px", borderRadius: 8, border: `1px solid ${ap.color}40`, background: ap.color+"10", color: ap.accent, cursor: "pointer", fontSize: 10, fontWeight: 600, fontFamily: "inherit" }}>🔑 Conectar API</button>
            )}
            {activeChat && <button onClick={() => deleteChat(activeChatId)} style={{ background: "none", border: "1px solid #2A2A30", color: "#888", cursor: "pointer", padding: "5px 10px", borderRadius: 8, fontSize: 11, fontFamily: "inherit" }}>🗑</button>}
          </div>
        </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "24px 20px", display: "flex", flexDirection: "column", gap: 16 }}>
          {!activeChat ? (
            <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 14, opacity: 0.4 }}>
              ...
              <div style={{ fontSize: 20, fontWeight: 600 }}>ChatHub</div>
            ...
            <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 12, opacity: 0.4 }}>
              <div style={{ fontSize: 42 }}>{broadcastMode?"📡":ap.icon}</div>
              <div style={{ fontSize: 16, fontWeight: 600 }}>{broadcastMode?"Broadcast":ap.name}</div>
        ...
        {activeChat && (
          <div style={{ padding: "10px 20px 16px", borderTop: "1px solid #2A2A30", background: "#18181C90", backdropFilter: "blur(12px)" }}>
            {!broadcastMode && (
              <div style={{ display: "flex", gap: 4, marginBottom: 8, overflowX: "auto" }}>
                {PROVIDERS.map(p => (
                  <button key={p.id} onClick={() => setActiveProvider(p.id)}
                    style={{ padding: "4px 10px", borderRadius: 20, border: activeProvider===p.id?`1px solid ${p.color}50`:"1px solid #2A2A30", background: activeProvider===p.id?p.color+"15":"transparent", color: activeProvider===p.id?p.accent:"#555", cursor: "pointer", fontSize: 10, fontWeight: 500, whiteSpace: "nowrap", display: "flex", alignItems: "center", gap: 4, fontFamily: "inherit" }}>
                    <span style={{ fontSize: 9 }}>{p.icon}</span>{p.name}
                    <span style={{ width: 4, height: 4, borderRadius: "50%", background: isConnected(p.id)?"#1ED99C":"#555" }} />
                  </button>
                ))}
              </div>
            )}
            <div style={{ display: "flex", gap: 10, alignItems: "flex-end" }}>
              <div style={{ flex: 1, background: "#1E1E22", borderRadius: 14, border: "1px solid #2A2A30", display: "flex", alignItems: "flex-end", padding: "4px 4px 4px 16px" }}>
                <textarea ref={inputRef} value={input} onChange={e => setInput(e.target.value)}
                  ...
                  placeholder={broadcastMode?"Enviar para todos...":`Mensagem para ${ap.name}...`}
                  rows={1}
...
export default function App() {
  ...
  useEffect(() => {
    const s = storageGet("session:current");
    if (s?.username) setUser(s.username);
    setChecking(false);
  }, []);

  const login = (u) => { storageSet("session:current", { username: u }); setUser(u); };
  const logout = () => { storageDelete("session:current"); setUser(null); };

  if (checking) return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#131316" }}>
      <div style={{ textAlign: "center", color: "#888", fontSize: 13 }}><div style={{ fontSize: 36, marginBottom: 12, animation: "float 2s ease-in-out infinite" }}>⚡</div>Carregando...</div>
    </div>
  );

  if (!user) return <AuthScreen onLogin={login} />;
  return <ChatApp username={user} onLogout={logout} />;
}
